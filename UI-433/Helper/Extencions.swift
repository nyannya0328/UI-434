...
extension View{
    ...
    func exportPDF<Content : View>(@ViewBuilder content : @escaping()->Content,competition : @escaping(Bool,URL?) -> ()){
        ...
        let outPutFiles = documentDirectory.appendingPathComponent("YOURENAME\(UUID().uuidString).pdf")
        ...
        do{
            ...
            try render.writePDF(to: outPutFiles, withActions: { context in
